...
class RMCSSubDeskVer1 : Component
{
  ...
  public virtual void OnStartBeforeUSBOpen(
    ref UInt16 pid,
    ref UInt16 vid,
    ref byte inEndpoint,
    ref byte outEndpoint
  ) { }
  ...
  public sealed override void Start()
  {
    ...
    if (_device == null || _device.IsInvalid)
      throw new Exception("usb Device Not Fount");

    var ret = MonoUsbApi.SetConfiguration(_device, _configuration);
    if (ret < 0)
      throw new Exception(
        string.Format(
          "Set Configuration Error: Vid:{0} , Pid:{1} , Endpoint:{2}, Configuration:{3}, Interface:{4},ErrorCode: {5}",
          _vid,
          _pid,
          _inEndpoint,
          _configuration,
    ...
    if (ret < 0)
      throw new Exception(
        string.Format(
          "Set Interface Error: Vid:{0} , Pid:{1} , Endpoint:{2}, Configuration:{3}, Interface:{4} ,ErrorCode: {5}",
          _vid,
          _pid,
          _inEndpoint,
          _configuration,
  ...
  private void USBReadLoop()
  {
    unsafe
    {
      ...
      var ret = MonoUsbApi.BulkTransfer(
        _device,
        _inEndpoint,
        (nint)header.Pointer,
        _readBuffer.BufferSizeOfByte,
      ...
      if (ret < 0)
        throw new Exception(
          string.Format(
            "Read Transform Error: Vid:{0} , Pid:{1} , Endpoint:{2}, Configuration:{3}, Interface:{4},ErrorCode: {5}",
            _vid,
            _pid,
            _inEndpoint,
            _configuration,
