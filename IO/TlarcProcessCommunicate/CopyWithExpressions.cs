...
namespace TlarcKernel.IO.TlarcMsgs;
...
public class CopyWithExpressions
{
  ...
  public void Copy(object src, object dst)
  {
    if (_copyDelegate == null)
    {
      ...
      var assignSource = Expression.Assign(
        sourceVariable,
        Expression.Convert(sourceParameter, sourceType)
      );
      var assignTarget = Expression.Assign(
        targetVariable,
        Expression.Convert(targetParameter, targetType)
      );

      var bindings = new List<Expression> { assignSource, assignTarget };
      ...
      foreach (var sourceFiled in sourceType.GetFields(BindingFlags.Public | BindingFlags.Instance))
      {
        if (sourceFiled.GetCustomAttribute<CopyToOtherProcessesAttribute>() is not null)
          continue;
        var targetFiled = targetType.GetField(
          sourceFiled.Name,
  ...
  public void NewCopy(object src, object dst)
  {
    if (_newCopyDelegate == null)
    {
      ...
      var body = Expression.Block(
        [sourceVariable, targetVariable, loopVariable],
        [.. bindings, targetVariable]
      );
