...
{
  public interface IReadOnlySubscription<T> : ISubscription
    where T : notnull
  {
    public SharedPtr<RentData<T>>? Rent { get; }
  }

  public interface IReadOnlyPublish<T> : IPublisher
    where T : notnull
  {
    public void LoadInstance(ref T instance);
  }

  public abstract class ReadOnlyInterfaceBase<T>(string InterfaceName)
    : IReadOnlySubscription<T>,
      IReadOnlyPublish<T>
    where T : notnull
  {
  ...
  class ReadOnlyReferenceInterface<T>(string InterfaceName)
    : ReadOnlyInterfaceBase<T>(InterfaceName)
    where T : class
  {
  ...
  class ReadOnlyValueInterface<T>(string InterfaceName) : ReadOnlyInterfaceBase<T>(InterfaceName)
    where T : struct
  {
    public override void LoadInstance(ref T instance)
    {
  ...
  class ReadOnlyUnmanagedInterfacePublisher<T>(string InterfaceName)
    : ReadOnlyInterfaceBase<T>(InterfaceName)
    where T : IDisposable
  {
    public override void LoadInstance(ref T instance)
  ...
  class ReadOnlyUnmanagedSubscription<T>(string InterfaceName)
    : IReadOnlySubscription<T>,
      ISubscriptionFormPublisher
    where T : IDisposable
  {
