...
partial class RingBuffer<THead>(int lengthOfByte)
  where THead : unmanaged
{
  ...
  public readonly unsafe int SizeOfHeader = sizeof(THead);

  public MemoryHandle HeadOfBuffer => _buffer[_writeOffset..].Pin();

  public void UpdateWritePointer(int dataLength) =>
    _writeOffset = (_writeOffset + dataLength) % BufferSizeOfByte;

  public ReadOnlyMemory<TTo> CastDataTo<TTo>(
    int dataLength,
    bool movePointerToNext = true,
    ...
    where TTo : unmanaged
  {
    ...
    if (dataLength > BufferSizeOfByte)
      throw new OutOfMemoryException(
        string.Format(
          "RingBuffer: Length of Output Data Length : {0} is Larger then BufferSizeOfByte : {1}",
          dataLength,
          BufferSizeOfByte
        )
      );
    ...
    unsafe
    {
      if (offset + dataLength > BufferSizeOfByte)
        Buffer.MemoryCopy(
          ...
          offset + dataLength - BufferSizeOfByte,
          offset + dataLength - BufferSizeOfByte
        );
    }
