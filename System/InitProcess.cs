...
namespace TlarcKernel.Init
{
  ...
  internal static class ProcessInit
  {
    internal static void Init(
      in IEnumerable<string> configFiles,
      ref Dictionary<uint, Process> processes,
      ref Dictionary<uint, ComponentCell> componentCells,
      ref Dictionary<Type, uint> lastInstance
    )
    {
      ...
      foreach (var i in files)
      {
        ...
        foreach (var property in processesProperties)
        {
          ...
          Dictionary<uint, ComponentCell> components = new() { { 0, new IOManager() } };
          ...
          foreach (var component in property.Components)
          {
            ...
            (d as Component).InitComponents(
              key,
              component.Value.Relies ?? [],
          ...
          processes.Add(
            ...
            {
              Pid = pid,
              Fps = property.Fps ?? 1000,
              Realtime = property.Realtime ?? false,
