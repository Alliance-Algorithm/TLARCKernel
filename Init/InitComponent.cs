...
namespace AllianceDM.Init
{
    ...
    {
        internal static void Init(ref Dictionary<uint, Component> components)
        {
            string path = DecisionMakerDef.ComponentsPath;

            components.Add(0, new IONode());
            ...
            foreach (var i in files)
            {
                try
                {
                    ...
                    var dj = new DataContractJsonSerializer(typeof(CompFiles));
                    var comp = dj.ReadObject(fs) ?? throw new Exception("Null Object");
                    var c = (CompFiles)comp;
